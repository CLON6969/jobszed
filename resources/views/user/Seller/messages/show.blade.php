...
@section('content')
<h2 class="text-2xl font-semibold mb-4">Chat with {{ $user->name }}</h2>
    ...
    @foreach($messages as $msg)
        ...
        @if($msg->sender_id === auth()->id())
            <div class="flex justify-end gap-2 mb-4">
                <a href="{{ route('user.Seller.messages.edit', $msg->id) }}" class="text-blue-600 text-sm">Edit</a>
                <form action="{{ route('user.Seller.messages.destroy', $msg->id) }}" method="POST" onsubmit="return confirm('Delete this message?')">
                    @csrf
                    @method('DELETE')
...
<form action="{{ route('user.Seller.messages.store') }}" method="POST" class="mt-4 flex gap-2">
    @csrf
